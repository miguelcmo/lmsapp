...
export function EditorProvider(props: EditorProviderProps) {
  const { provider, ydoc, placeholder = "Start writing...", aiToken, content, onChange, enableCollaboration = false } = props
  ...
  const editor = useEditor({
    ...
    extensions: [
      StarterKit.configure({
        undoRedo: !enableCollaboration, // Habilitar undo/redo solo si no hay colaboración
      ...
      ...(enableCollaboration ? [Collaboration.configure({ document: ydoc })] : []),
      ...(enableCollaboration ? [CollaborationCaret.configure({
        provider,
    ...
    onUpdate: ({ editor }) => {
      // ✅ Capturar cambios y enviar al parent
      if (onChange) {
...
export function NotionEditor({
  ...
  return (
    <UserProvider>
      <CollabProvider room={room} enabled={enableCollaboration}>
        <AiProvider>
          <TocProvider>
            <NotionEditorContent 
 ...
 */
export function NotionEditorContent({ placeholder, content, onChange, enableCollaboration = false }: { placeholder?: string; content?: any; onChange?: (content: any) => void; enableCollaboration?: boolean }) {
  ...
  if ((enableCollaboration && !provider) || !aiToken) {
    return <LoadingSpinner />
  }
